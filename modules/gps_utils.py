...
def clean_gps_data(gps_data):
    ...
    return {
        ...
        "longitude": (
            float(gps_data["longitude"]) if gps_data.get("longitude") else None
        ),
        "altitude": (
            convert_to_float(gps_data["altitude"]) if gps_data.get("altitude") else None
        ),
    }
...
def save_location_data(image_file, disease_name, confidence, gps_data):
    """Save disease detection results along with GPS data."""
    ...
    if not gps_data:
        ...
        try:
            ...
            if latitude is not None and longitude is not None:
                location = f"Location at ({latitude:.6f}, {longitude:.6f})"
            else:
                location = "Location unknown"
    ...
    # Format notes with confidence and GPS status
    notes = f"Confidence: {confidence:.1f}%"
