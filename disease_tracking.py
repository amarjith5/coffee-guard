...
def main(theme_colors=None):
    st.info("Disease markers may overlap; zoom in for a clearer view.", icon="ℹ️")
    ...
    required_columns = ["disease", "latitude", "longitude", "timestamp"]
    ...
    with col2:
        with st.container(border=True):
            ...
            disease_filter = st.selectbox(
                "Select Disease Type", ["All"] + sorted(df["disease"].unique())
            )
    ...
    with col1:
        with st.container(border=True):
            if len(filtered_df) > 0:
                filtered_df["color"] = filtered_df["disease"].apply(
                ...
                fig = px.scatter_mapbox(
                    ...
                    hover_name="disease",
                    hover_data=["location", "status", "notes"],
                    zoom=5,
                    size_max=15,
                    opacity=0.8,
                )
        ...
        with st.container(border=True):
            ...
            if not filtered_df.empty:
                time_series = (
                    filtered_df.groupby(
                        [filtered_df["timestamp"].dt.to_period("M"), "disease"]
                    )
                    .size()
